        ...
        x86_64-linux =
          let
            supportedConfigurations = lib.listToAttrs (map
              (c: {
                name = "${c.som}" + (lib.optionalString (c.super or false) "-super") + "-${c.carrierBoard}" + (lib.optionalString (builtins.hasAttr "majorVersion" c) "-jp${c.majorVersion}");
                value = c;
              }) [
